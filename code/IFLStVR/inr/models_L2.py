...
class NeSVoR(nn.Module):
    def __init__(
       ...
       # self.psf_sigma = resolution2sigma(resolution, isotropic=True)
    ...
    def forward(
        ...
        if not self.args.no_slice_variance:
            ...
            #new l2
            log_var_slice = self.log_var_slice.exp()[slice_idx]
            log_var_loss = (abs(log_var_slice)).mean()
            print('log_var_loss')
            print(self.log_var_slice.exp()[slice_idx])
        ...
        #v_out_norm = (v_out - v_out.min()) / (v_out.max() - v_out.min())
        #v_norm = (v - v.min()) / (v.max() - v.min())
        ##计算 SSIM
        #import torchvision
        #ssim_value = torchvision.transforms.functional.ssim(v_out_norm.unsqueeze(0), v_norm.unsqueeze(0), data_range=1.0)
       # 使用 SSIM 替换 D_LOSS
        #losses = {D_LOSS: 1 - ssim_value}
