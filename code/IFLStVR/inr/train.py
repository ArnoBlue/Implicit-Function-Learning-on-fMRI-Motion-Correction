...
def train(slices: List[Slice], args: Namespace) -> Tuple[INR, List[Slice], Volume]:
    ...
    # Create a list to store the total loss every 100 iterations
    saved_losses = []
    ...
    for i in range(1, args.n_iter + 1):
        ...
        average.update('loss', loss.item())
        
        # Save total loss every 100 iterations
        if i % 100 == 0:
            saved_losses.append(average['loss'])
    ...
    # Print all saved losses after training
    print("Saved losses every 100 iterations:")
    for idx, saved_loss in enumerate(saved_losses, 1):
        print(f"Iteration {idx*100}: Loss = {saved_loss}")
